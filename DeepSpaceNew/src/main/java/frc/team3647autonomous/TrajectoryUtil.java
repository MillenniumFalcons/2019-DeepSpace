...
public class TrajectoryUtil {
    ...
    public static Trajectory correctPath(Trajectory originalTrajectory) {
        ...
        return new Trajectory(segments.stream()
                .map(segment -> new Segment(segment.dt, segment.x, Constants.kFieldWidth - segment.y, segment.position,
                        segment.velocity, segment.acceleration, segment.jerk, -segment.heading))
                .toArray(Segment[]::new));
    }
    ...
    public static Trajectory getTrajectoryFromName(String trajectoryName) {
        ...
        if (trajectory == null) {
            ...
            Robot.runAuto = false;
            // trajectoryFile = new
            // File("C:\\Users\\brian\\OneDrive\\Projects\\FRC_2018_Offseason\\PathPlanner\\Trajectories\\"
            // + trajectoryName + "\\" + trajectoryName + "_source_detailed.traj");
            // trajectory = trajectoryFile.exists() ?
            // Pathfinder.readFromFile(trajectoryFile): null;
        } else
